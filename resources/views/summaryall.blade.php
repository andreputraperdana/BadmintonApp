...
<html lang="en">
  <head>
    ...
    {{-- <script defer src="summaryiuran.js"></script> --}}
  </head>
  <body>
    ...
    <div style="max-width: 100vw; display: flex; justify-content: center">
      <div style="width: 90%;">
      <form action="/summaryall" method="POST">
          @csrf
          <fieldset class="border ps-3" style="width: 700px; border: solid 1px black !important;">
              <legend class="lejen w-auto">Summary All</legend>
              <div class="raja">
                <div class="apaaja">
      ...
      <table class="table table-bordered border-dark" style="overflow: scroll; max-width: 90vw">
          <thead>
              <tr>
                  ...
                  <th scope="col" style="text-align: center;">Uang Keluar</th>
          ...
          @foreach ($all as $alls)
            <tbody>
              <tr>
                <td style="text-align: center;">{{ $alls->bulan }}</td>
                <td style="text-align: center;">{{ date('d-m-Y', strtotime($alls->created_at)) }}</td>
                <td style="text-align: center;">@currency($alls->totalUangMasuk) </td>
                <td style="text-align: center;">@currency($alls->totalUangKeluar)</td>
                <td style="text-align: center;">@currency($alls->totalUangMasuk - $alls->totalUangKeluar)</td>
              </tr>
            </tbody>
            @php
              $totalSelisih += $alls->totalUangMasuk - $alls->totalUangKeluar;
              $totalUangMasuk += $alls->totalUangMasuk;
              $totalUangKeluar += $alls->totalUangKeluar;
            @endphp
          @endforeach
            <tfoot>
              <tr>
                ...
                <td></td>
                <td style="text-align: center;">@currency($totalUangMasuk)</td>
                <td style="text-align: center;">@currency($totalUangKeluar)</td>
                <td style="text-align: center;">@currency($totalSelisih)</td>
              </tr>
            </tfoot>
      </table>
    </div>
